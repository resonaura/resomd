  ...
  function MarkdownPreview({ content, onSelectBlock }, ref) {
    ...
    const handleClickBlock = useCallback(
      (node: ExtraProps['node'], e: React.MouseEvent) => {
        ...
        if (start !== undefined && end !== undefined && onSelectBlock) {
          ...
          console.warn(
            'handleClickBlock: start/end or onSelectBlock missing!',
            { start, end, hasSelect: !!onSelectBlock }
          );
        }
      },
    ...
    const markdownComponents: Components = useMemo(() => {
      return {
        ...
        li: ({ node, children }) => (
          <li onClick={e => handleClickBlock(node, e)}>{children}</li>
        ),
        pre: ({ node, children }) => (
